...
public class VarExe7 {
	...
	public static void main(String[] args) {
		...
		boolean run = true; // Member 값을 저장.
		Member[] storage = new Member[100]; // {null, null, ... null}s
		...
		while (run) {
			...
			case 1: // 등록.
				...
				Member member = new Member(); // 인스턴스생성
				// member.name = name;
				// member.score = score;
			...
			case 3: // 삭제. 이름입력 조회 후 => null 대입.
				...
				name = scn.nextLine();
				boolean isExist1 = false; // 존재여부를 체크
				for (int i = 0; i < storage.length; i++) {
					if (storage[i] != null && storage[i].getName().equals(name)) {
						storage[i] = null; // 삭제.
